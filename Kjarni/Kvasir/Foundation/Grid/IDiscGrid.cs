...
namespace Kjarni.Kvasir.Foundation.Grid;
...
public interface IDiscGrid : IGrid
{
    ...
    CellId CellAt(Coords2D position, Resolution resolution) => CellAt(position.X, position.Y, resolution);

    /// <summary>Returns the centre coordinate of a cell.</summary>
    Vector2 CenterOf(CellId cell);

    /// <summary>Returns the boundary polygon vertices of a cell in traversal order.</summary>
    Vector2[] BoundaryOf(CellId cell);
}
